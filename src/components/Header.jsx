...
const Header = () => {
  ...
  return (
    <div
      className="fixed z-10 flex justify-between  items-center h-24 w-full max-w-[1240px] mx-auto px-4 text-white"
      ref={headerRef}
    >
      <div className="">
        <Link to="/">
      ...
      <ul className=" hidden sm:flex items-center justify-end w-1/2 space-x-10">
        {headerNav.map((e, i) => (
          <li
            key={i}
      ...
      <div onClick={handleNav} className="block sm:hidden">
        {nav ? (
          <RiCloseFill className="text-white" size={20} />
        ) : (
          <FcMenu className="text-white" size={20} />
        )}
      </div>
      <ul
        className={
